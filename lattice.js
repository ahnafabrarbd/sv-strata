...
const edgeRecords = new Map();   // edgeId  -> THREE.Line
...
function buildPlane(p) {
    ...
    // Click target — translucent fill plane lying flat (XZ).
...
function addNodeMesh(n) {
    ...
    if (!plane) return;
    const group = planeGroups.get(n.planeId);
    if (!group) return;

    const geom = new THREE.SphereGeometry(NODE_R, 18, 18);
    const mat = new THREE.MeshBasicMaterial({ color: new THREE.Color(plane.color) });
...
function addEdgeLine(e) {
    ...
    if (!a || !b) return;
    const ap = state.planes.find(p => p.id === a.planeId);
    const bp = state.planes.find(p => p.id === b.planeId);
    if (!ap || !bp) return;
    const isCross = a.planeId !== b.planeId;
    const geom = new THREE.BufferGeometry().setFromPoints([
        new THREE.Vector3(a.x, ap.y + 2, a.z),
        new THREE.Vector3(b.x, bp.y + 2, b.z)
    ]);
    const mat = isCross
        ...
        : new THREE.LineBasicMaterial({
            color: new THREE.Color(ap.color), transparent: true, opacity: 0.55
        });
    ...
    line.renderOrder = 2;
    scene.add(line);
    edgeRecords.set(e.id, line);
}

function disposeEdge(edgeId) {
    const line = edgeRecords.get(edgeId);
    if (!line) return;
...
function updateEdgeGeometry(e) {
    const line = edgeRecords.get(e.id);
    if (!line) return;
    const a = state.nodes.find(n => n.id === e.fromId);
    const b = state.nodes.find(n => n.id === e.toId);
    if (!a || !b) return;
...
renderer.domElement.addEventListener('pointermove', e => {
    ...
    if (!node) return;
    const plane = state.planes.find(p => p.id === node.planeId);
    if (!plane) return;
    setNDC(e.clientX, e.clientY);
    raycaster.setFromCamera(ndc, camera);
    const point = intersectPlaneY(plane.y);
    if (!point) return;
    node.x = point.x;
    node.z = point.z;
    const rec = nodeRecords.get(node.id);
    if (rec) {
        rec.mesh.position.x = node.x;
        rec.mesh.position.z = node.z;
        rec.sprite.position.x = node.x;
        rec.sprite.position.z = node.z;
    }
...
function handleConnectClick(nodeId) {
    ...
    refreshSelectionVisuals();
    updateBanner();
    if (selectedId) renderEdgesPanel(state.nodes.find(n => n.id === selectedId));
}
...
function updateBanner() {
    ...
    banner.textContent = pendingFromId
        ? 'Click a second node to connect — any plane'
        : 'Connect mode — click two nodes (same plane or across planes)';
}

function refreshSelectionVisuals() {
    nodeRecords.forEach((rec, id) => {
        const node = state.nodes.find(n => n.id === id);
        const plane = node && state.planes.find(p => p.id === node.planeId);
        if (!plane) return;
        if (id === pendingFromId) {
            ...
            rec.mesh.material.color.set(new THREE.Color(plane.color));
        } else {
...
function populatePlaneSelect(n) {
    ...
    state.planes.forEach(p => {
        ...
        if (p.id === n.planeId) opt.selected = true;
...
function renderEdgesPanel(n) {
    ...
    incident.forEach(e => {
        ...
        planeTag.textContent = otherPlane ? otherPlane.label : '';
        if (otherPlane) planeTag.style.color = otherPlane.color;
...
npPlane.addEventListener('change', () => {
    ...
    const n = state.nodes.find(x => x.id === selectedId);
    if (!n) return;
    const newPlaneId = npPlane.value;
    if (newPlaneId === n.planeId) return;
    const newPlane = state.planes.find(p => p.id === newPlaneId);
    if (!newPlane) return;
    n.planeId = newPlaneId;
    const rec = nodeRecords.get(n.id);
    if (rec) {
        if (rec.mesh.parent) rec.mesh.parent.remove(rec.mesh);
        if (rec.sprite.parent) rec.sprite.parent.remove(rec.sprite);
        const group = planeGroups.get(newPlaneId);
        group.add(rec.mesh);
        group.add(rec.sprite);
        rec.mesh.material.color.set(new THREE.Color(newPlane.color));
    }
...
function importLayerInto(plane) {
    ...
    catch (e) { alert('Could not parse stratum data.'); return; }
    ...
    // 2D nodes use SVG coords roughly in 0..540. Center them on the plane.
    const idMap = {};
